# RUNBOOK-031: External Model Server mTLS Setup
...
```bash
# From Hliðskjálf platform, transfer using one of these methods:

# Option 1: AirDrop (recommended for Mac-to-Mac)
open -R scripts/setup-model-server-macos.sh
# Use Finder's AirDrop feature to send to external Mac

# Option 2: Temporary HTTP server (if no SSH)
cd scripts
python3 -m http.server 8888
# Note your IP address, then on external machine:
...
The script handles:
- Downloading correct binaries (darwin-arm64)
- Creating configurations
- Setting up launchd services
- Starting SPIRE agent with join token
- Configuring spiffe-helper

Skip to [Step 4](#step-4-configure-model-service-with-svid) if using the automated script.

---

**For Linux/Manual Setup:**

On `192.168.50.26`:

```bash
# Download SPIRE (for macOS ARM64 - M1/M2/M3/M4/M5)
...
# For macOS: Create launchd service
...
# For Linux: Create systemd service
...
```bash
# Install spiffe-helper (for macOS ARM64)
