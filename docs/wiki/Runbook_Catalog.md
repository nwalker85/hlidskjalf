...
| [RUNBOOK-024-add-shared-service.md](docs/runbooks/RUNBOOK-024-add-shared-service.md) | `infra`, `routing`, `mtls` | Reserve ports, add compose entries, configure Traefik/SPIRE | Port registry edit rights, Docker/Traefik access |
...
| [RUNBOOK-025-norns-work-queue.md](docs/runbooks/RUNBOOK-025-norns-work-queue.md) | `automation`, `gitlab`, `ai` | Route GitLab issues to Norns via Bifrost, maintain webhooks, monitor queue health | GitLab PAT + webhook secret in LocalStack, Bifrost deployed, `actor::norns` label |
| [RUNBOOK-025-host-ollama.md](docs/runbooks/RUNBOOK-025-host-ollama.md) | `ai`, `ollama`, `gpu` | Run Ollama on host GPU, expose to Docker, update env vars | Apple Silicon macOS, Homebrew, Docker Desktop |
| [RUNBOOK-026-llm-configuration-system.md](docs/runbooks/RUNBOOK-026-llm-configuration-system.md) | `ai`, `llm`, `configuration`, `database` | Setup, operate, and troubleshoot database-backed LLM provider and model configuration | PostgreSQL, LocalStack Secrets Manager, LLM provider API access |
| [RUNBOOK-026-letsencrypt-external-https.md](docs/runbooks/RUNBOOK-026-letsencrypt-external-https.md) | `infra`, `security`, `certificates`, `traefik`, `staging` | Configure Let's Encrypt for staging `.dev` environment, multi-environment HTTPS setup | DNS configured, ports 80/443 forwarded, Zitadel applications configured |
| [RUNBOOK-026-m2c-demo-integration.md](docs/runbooks/RUNBOOK-026-m2c-demo-integration.md) | `infra`, `work`, `ports` | M2C demo port registry integration, env config, Traefik routing | Port registry access, Docker Compose |
| [RUNBOOK-027-sip-voice-platform.md](docs/runbooks/RUNBOOK-027-sip-voice-platform.md) | `voice`, `sip`, `telephony`, `livekit`, `work` | Deploy SIP voice AI platform, inbound/outbound calling, agent orchestration, on-prem readiness | LiveKit server, Twilio SIP trunk, Deepgram/ElevenLabs/OpenAI APIs, port registry |
| [RUNBOOK-028-dashboard-real-data.md](docs/runbooks/RUNBOOK-028-dashboard-real-data.md) | `observability`, `dashboard`, `api`, `data` | Integrate dashboard with real-time data, configure background jobs, optimize queries | Database running, port_registry.yaml configured, Docker API access |
| [RUNBOOK-028-mcp-zitadel-jwt.md](docs/runbooks/RUNBOOK-028-mcp-zitadel-jwt.md) | `identity`, `mcp`, `oauth` | Obtain Zitadel JWTs with MCP scopes, use them in MCP `/tools` calls | Zitadel project access, OAuth client credentials, MCP endpoint |
| [RUNBOOK-029-serviceasanagent-integration.md](docs/runbooks/RUNBOOK-029-serviceasanagent-integration.md) | `work`, `voice`, `infra` | Register Service-as-an-Agent demo, align compose networks, wire Traefik domains | Port registry update rights, `platform_net` provisioned, Traefik reload access |
| [RUNBOOK-030-compose-management.md](docs/runbooks/RUNBOOK-030-compose-management.md) | `infra`, `docker`, `operations` | Manage modular compose stacks, start/stop services independently, optimize dev workflow | Docker Compose, modular stack understanding |
| [RUNBOOK-031-external-model-server-mtls.md](docs/runbooks/RUNBOOK-031-external-model-server-mtls.md) | `security`, `mtls`, `spire`, `external` | Configure mTLS for external model servers using SPIRE, setup agent on remote machines | SPIRE server running, access to external machine, network connectivity |
| [RUNBOOK-032-external-model-server-nginx.md](docs/runbooks/RUNBOOK-026-external-model-server.md) | `ai`, `nginx`, `external`, `model-server` | Setup external Mac workstation as model server with nginx reverse proxy | MacBook Pro with model server, nginx, DNS configured |
